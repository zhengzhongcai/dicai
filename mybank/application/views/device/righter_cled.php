	...
	<div class="right_box">
		...
		<div class="mob_tool">
			...
			<select id="deviceType" name="deviceType" onchange="changeDevice()">
				...
				<option value="Cled" selected>窗口LED</option>
...
<script>
	...
	function addParamInfo(){
		...
		if(isNaN($('input[name="cLed_addr"]').val()) || parseInt($('input[name="cLed_addr"]').val())>999){
			alert('呼叫器地址必须为数值，且不大于999');
			return;
		}
		var postData = getPostData($('#cled_pop_box .tbAddParamInfo'));
		var url = "<{$baseUrl}>/index.php?control=device&action=addCledInfo";
		$.ajax({
			type: "post",
			url: url,
			dataType:'json',
	...
	function getCledInfos(){
		var deviceStr = "";
		var orgId = $('#orgId').val();
		var url = "<?php echo $session['baseurl']; ?>/index.php?control=device&action=getCledInfos";
		$.ajax({
			type: "post",
			url: url,
			dataType:'json',
	...
	function saveParamInfo(){
		...
		var url = "<?php echo $session['baseurl']; ?>/index.php?control=device&action=saveCledInfo";
		$.ajax({
			type: "post",
			url: url,
			data: postData + "paramId=" + paramInfo.cLed_mac + '&orgId='+updateOrgId+'&new_sysno='+org_radio_val,
			success: function(data) {
				if (data > 0)
				{
					alert('修改成功.');
	...
	function deleteDeviceInfo(){
		...
		var url = "<?php echo $session['baseurl']; ?>/index.php?control=device&action=deleteCledInfo";
		$.ajax({
			type: "post",
			url: url,
			dataType:'json',

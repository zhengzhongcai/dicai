...
function getClientsInfo(){
		$.ajax({
			url:"index.php/client/getClientInfoAjax",
			type:"post",
...
function clientAjax(dataAll,str_clientID){
	...
	$.ajax({
			url:"index.php/socket/controlClients",
			type:"post",
			data:{data:dataAll},
			success:function(str){
				var result=str;
...
function AddClientPlayList(dataAll){
	$.ajax({
			url:"index.php/client/AddClientPlayList",
			type:"post",
...
function setDownLoadTime(){
	...
	if(clientID==''){
	   ...
	   var ajax = new AJAXRequest();
		...
		ajax.post("index.php/client/setDownLoadTime", 
...
function setFTP()
{
	...
	var ajax = new AJAXRequest();
		...
		ajax.post("index.php/client/setclientftp", 
		tdata,
		function(obj)
		{
			var st=obj.responseText;
...
function upadteName(b,v,o)
{
	var ajax = new AJAXRequest();
		...
		ajax.post("index.php/client/updateClientName", 
		'id='+b+"&name="+v,
		function(obj)
		{
			var st=obj.responseText;
...
function getClientStatus(st){
	if(st)
	{
	   ...
	   var ajax = new AJAXRequest();
		...
		ajax.post("index.php/socket/getClientState", 
		"",
		function(obj)
		{
			...
			if(result.indexOf("Register fail")>=0){
				...
				bug("用户退出登录",top.location+"<br>"+self.location);
					...
					{
						 top.location="index.php/c_login/loginOut";
					}
				return false;
			}
		...
		function(obj)
		{
			...
			if(result.indexOf("Register fail")>=0){
				...
				bug("用户退出登录",top.location+"<br>"+self.location);
					if (top.location !== self.location) 
					{
						 top.location="index.php/user/loginOut";
					}
				return false;
			}
...
function submitShengpiInfo()
{
	...
	if(!err)
	{
			...
			ajax.post("index.php/remind/insertRemind/","arg="+info, function(obj)
			{
				var st=obj.responseText;
...
function viewClient()
{
	...
	setTimeout(function(){
		...
		{
			size(w,h+20);
			content('<iframe style="display:block; width:'+w+'px; height:'+h+'px; overflow:hidden"  src="index.php/vncView/viewVnc/'+encodeURIComponent(str)+'" frameborder="0" allowtransparency="true"></iframe>');
			position();
		}
		},200);
}
...
function createrClientList(newClientInfo)
{
	...
	for(var i in newClientInfo)
	{
		...
		str=str+'<span class="c_checkBox"><input state="unline" type="checkbox" pNode="'+pNode+'" rowId="'+newClientInfo[i].clientNodeCode+'" ctype="1" info="'+info+'" name="clientID" value="'+newClientInfo[i].clientNum+'"/></span>';
        ...
        str=str+'<span class="c_mshu" ondblclick="createInput(event)">'+newClientInfo[i].clientName+'</span>';
...
function getScreenshotPhoto(str_ftpId,str_mac)
{
	...
	ajax.post("index.php/c_clientControl/getScreenshotPhoto",'arg={"clientFTP":"'+str_ftpId+'","clientFolder":"'+str_mac+'"}', function(obj)
	{
		var st=obj.responseText;
		bug("服务器返回的信息",st,"green");
		st=eval("("+st+")");
...
var clientManage={
	...
	readRiZhi:function(clientId,clientName){
		...
		art.dialog.open('index.php/client/getClientLogs/'+clientId, {title: '终端->'+clientName+"日志",resize:false,drag:false,lock:true,width:"95%",height:"95%"});
	}
};

	...
	<div class="right_box">
		<!----tool 工具条---->
		<div class="mob_tool">
			<a auth="c" href="javascript:showPopbox('addRes')" class="tool_add statItem">添加图片</a>
...
<script>
	...
	function generateRes(res){
		...
		var paramStr = '<tr>'+
						  ...
						  '<td>'+res.id+'</td>'+
						  '<td>'+status+'</td>'+
						  '<td title="点击预览"><span style="cursor:pointer" onclick="showRes(\''+res.name+'\',\'<{$baseUrl}>/index.php?control=resource&action=checkRes&resid='+res.id+'\')">'+res.name+'</span></td>'+
						  '<td>'+res.size+'</td>'+
						  '<td>'+res.path+'</td>'+

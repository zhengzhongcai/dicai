...
colorMap.prototype.ColorTable=function(userFunction)
{
	...
	var tr ,td, fun_click,fn_mouseOver;
	for(var i=0; i<colorAy.length; i++)
	{
		tr=document.createElement("TR");
		for(var a=0; a<colorAy[i].length; a++)
		{
			td=document.createElement("TD");
			td.id=i+"_"+a;
			td.setAttribute("color","#"+colorAy[i][a]);
			with(td.style)
			{
				backgroundColor="#"+colorAy[i][a];
				width="12px";
				height="12px";
			}
			fn_mouseOver=function(id,ColorPreviewId,ColorHexId)
			{
				return function()
				{
					document.getElementById(ColorPreviewId).style.backgroundColor=document.getElementById(id).style.backgroundColor;
					document.getElementById(ColorHexId).value=document.getElementById(id).getAttribute("color");
				};
			};
			fun_click=function(colorValue)
			{
				return function(){userFunction(colorValue);};
			};
			td.attachEvent("onclick",fun_click("#"+colorAy[i][a]));
			td.attachEvent("onmouseover",fn_mouseOver(i+"_"+a,"ColorPreview"+this.id,"ColorHex"+this.id));
			td.innerHTML=" ";
			tr.appendChild(td);
		}
		tb.appendChild(tr);
...
colorMap.prototype.getColorTable=function()
{
	var colorTBLE="<table border='0' cellspacing='0' cellpadding='2' width='100%'>\
 <tr>\
  <td  valign=center><div style=\"background-color: #000000; padding: 1px; height: 21px; width: 50px\"><div id='ColorPreview"+this.id+"' style=\"height: 100%; width: 100%\"></div></div></td>\
  <td  valign=center><input type='text' name='ColorHex"+this.id+"' id='ColorHex"+this.id+"' value='' size=15 style=\"font-size: 12px\"></td>\
  <td  width=100%></td>\
 </tr>\
</table>\
	";

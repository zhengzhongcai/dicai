...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		...
		//this.startUpload();
		//检测文件是否存在
		resources.upload.checkFileNameByList();
	} catch (ex)  {
        this.debug(ex);
	}
}
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	try {
		...
		if(percent==100){
			//每个文件上传完后的处理
			resources.upload.tipEveryUpload(file);	
		}
	} catch (ex) {
		this.debug(ex);
	}
...
function uploadSuccess(file, serverData) {
	try {
		...
		progress.toggleCancel(false);
			...
			//处理已上传完成后返回的数据
        //_("txtFileName").value=file.name;
		//dealWith(serverData);
		resources.upload.uploadSucessEveryOne(serverData,file);
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			//在resources.upload.fileNameList 中去掉缓存信息
			resources.upload.delFileCache(resources.upload.fomartFileName(file.name));
			break;
...
function queueComplete(numFilesUploaded) {
	...
	//status.innerHTML = numFilesUploaded + " 个" + (numFilesUploaded === 1 ? "" : "") + " 文件上传成功.";
	
	resources.upload.queueUploadSucess(numFilesUploaded);
}
...
function swfUploadLoaded() {
	...
	document.getElementById("btn_uploadStart").onclick = function () { self.startUpload(); };
}
